...
class StufgeoGML:
    ...
    def gen_labels(self,element):
        texts,rotations,points,gml_points = xml_utils.get_labels(element)
    ...
    def topo_feature(self,element,imgeo_geom):
        ...
        lokaal_id = xml_utils.get_lokaal_id(element)

        if geom_type == "Point":
            feature_obj = GML.featureObject("TopoPoint",gml_geom,lokaal_id)
            feature_obj.append(GML.featureAttribute("Type",element.find(IMGEO_PLUSTYPE).text))
        elif "Line" in geom_type:
            feature_obj = GML.featureObject("TopoLine",gml_geom,lokaal_id)
        else:
            feature_obj = GML.featureObject("Topo",gml_geom,lokaal_id)
